...
public class RecurrenceDataBean {
    ...
    public int recurrence_frequency_type = FREQ_NONE;// 重复频率类型
    public int recurrence_interval = -1;// 重复间隔 (未设置-1)
    public int recurrence_end_type = END_TYPE_NONE;// 重复截止类型
    public long recurrence_end_date = -1;// 重复截至时间戳,until
    public int recurrence_end_count = -1;// 重复截至次数 (未设置-1)
    ...
    public int recurrence_week_start = WKST_NONE;// 周首日 1 SUN 2Mon
    ...
    public RecurrenceDataBean(int eventId, RRule rRule, long startDate) {
        ...
        this.recurrence_smonth = calendar.get(Calendar.MONTH) + 1;//1-12month
        this.recurrence_sday = calendar.get(Calendar.DAY_OF_MONTH);
